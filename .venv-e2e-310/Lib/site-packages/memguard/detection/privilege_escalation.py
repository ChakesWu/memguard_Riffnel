...
def _strip_entities(text: str) -> str:
    ...
    text = BANK_ACCOUNT_PATTERN.sub(" ", text)
    return text
...
class PrivilegeEscalationDetector(BaseDetector):
    ...
    def check_write(
        ...
        keyword_text = _strip_entities(new_text)

        # ── Content scan (always runs) ──
        # Catches dangerous content regardless of history state.
        content_result = self._scan_content_only(entry, new_text, keyword_text)
        ...
        old_keyword_text = _strip_entities(old_text)

        score = 0.0
        reasons = []

        # Privilege keyword escalation (substring matching)
        old_priv = {kw for kw in PRIV_KEYWORDS if kw in old_keyword_text}
        new_priv = {kw for kw in PRIV_KEYWORDS if kw in keyword_text}
        ...
        old_fin = {kw for kw in FINANCIAL_KEYWORDS if kw in old_keyword_text}
        new_fin = {kw for kw in FINANCIAL_KEYWORDS if kw in keyword_text}
    ...
    def _scan_content_only(
        self, entry: MemoryEntry, text: str, keyword_text: str
    ) -> DetectionResult:
        ...
        priv_in_content = {kw for kw in PRIV_KEYWORDS if kw in keyword_text}
        ...
        fin_in_content = {kw for kw in FINANCIAL_KEYWORDS if kw in keyword_text}
